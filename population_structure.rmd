...
I'll read in the clinical data, and re-load the qc-data.
...
res <- svd(t(X_scaled), nu = 2, nv = 0)
# nv = 0 indicates that I am not interested in computing right-singular vectors - these are not needed for PCA 
# nu = 2 indicates that I am interested in only the first two PCs

## Compute the principal components
(pca <- X_scaled %*% res$u)[1:2]


# NB: using the 'prcomp' function would be another comparable approach here. 
```

### Using another appraoch

  At the time of this writing, the code above makes my `R` session crash. 
As an alternative approach, I can use the function `std` from the `ncvreg` package. 
...
# Let's try using the Rspectra package 

rm(X) # remove unscaled X object to save memory
library(RSpectra)
?svds()
# Use singular value decomposition to derive the left singular vectors 
singular_vectors <- svds(X_scaled,
                         k = 2,
                         nu = 2, # NB: default is nu = k
                         nv = 0)

# calculate the PCs                          
PCs <- t(X_scaled) %*% singular_vectors$u

# create a plot of the first two PCs
PC_plot <- plot(PCs[,1], PCs[,2], xlab="Principal Component 1", ylab="Principal Component 2")

(PC_plot)
```

```{r pca_with_rsvd, eval=FALSE}

# library(rsvd) # Paackage for randomized singular value decomposition 

pca <- rsvd::rpca(A = X, # the matrix of SNP data
