...
If we construct and plot some of the top PCs of our data, we should see clustering that corresponds to known, or unknown, population structure. Our data will require special tools for PCA because of its size. We could manually compute the pcs, but would probably 
need to run it on the cluster with a lot of memory, and it  would take a long time.  

We'll be using tools from the package `SNPRelate` which allow us to do key computations quickly. To use `SNPRelate` functions, we need to get our data in a GDS format. `SNPRelate`
has a function `snpgdsBED2GDS()` to convert PLINK binary data into a GDS file, but as 
far as I am aware, there is no tool to convert `SnpMatrix` objects to GDS. So, we need 
to first convert our qc data back to bed/bim/fam using the `snpStats` function `write.plink()`, and then use that to create a GDS file:

$$\text{SnpMatrix} \longrightarrow \text{bed/bim/fam} \longrightarrow \text{GDS}$$

There is code for this in gwas-3, but I'll include it here for completeness. I'll read in the clinical data, and re-load the qc-data because we replaced the original copy with the imputed version.
```{r}
...
set with absolutely no missingness, we could do
$$\text{bed/bim/fam} \longrightarrow \text{SnpMatrix} \longrightarrow \text{bed/bim/fam} \longrightarrow \text{GDS}$$
Yes, this is annoying.

Back to our qc-d plink data. We will now create our GDS file, use `SNPRelate` to
compute the PCs, and plot them to see if it looks like there is any kind of sample
structure.
```{r}
...
# put top 10 pcs in a table - 10 is somewhat arbitrary but based on the scree plot seems reasonable
...
<!-- We'll get there, but to start, I'm going to illustrate the concept using some smaller data with known structure. -->

<!-- First, read in the data and filter out the monomorphic SNPs. Note that this data set contains a known race variable. -->
<!-- ```{r, process_admixture} -->
...
<!-- plot(g) -->
<!-- ``` -->
<!-- We can see that PC1 seems to differentiate  -->
