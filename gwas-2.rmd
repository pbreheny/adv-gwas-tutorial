...
```{r save_X_numeric, eval=FALSE, include=FALSE}
# saveRDS(rules_imputed_numeric, "data/rules_imputed_numeric.R")
rules_imputed_numeric <- readRDS("data/rules_imputed_numeric.R")
```

```{r numeric_call_rates, include=FALSE, eval=FALSE}
...
```{r working_code, eval=FALSE, include=FALSE}
...
# Another approach 
# fully_imputed_numeric <- matrix(nrow = nrow(rules_imputed_numeric),
#                                 ncol = ncol(rules_imputed_numeric))
# 
# for (j in 1:ncol(fully_imputed_numeric)) {
#   
#   if(j %in% to_impute){
#     fully_imputed_numeric[, j, drop=FALSE] <- apply(X = rules_imputed_numeric[, j, drop=FALSE],
#                                        MARGIN = 2,
#                                        FUN = impute_mean)
#   } else {
#     fully_imputed_numeric[, j, drop=FALSE] <- rules_imputed_numeric[, j, drop=FALSE]
#   }
#   
# }
...
# .Data is a matrix containing values of type 'raw'
fully_imputed@.Data[, to_impute] <- sapply(1:length(to_impute), function(x){
  ...
  # I need numeric values in order to calculate the mean 
  s <- as(fully_imputed[, to_impute][,x], 'numeric')
  idx <- which(is.na(s)) # identify missing values in a numeric vector
  s[idx] <- mean(s, na.rm = TRUE) # replace missing values with that SNP mean
  raw <- snpStats::mean2g(s) # convert the numeric SNP vector back to its raw, memory-friendly form
  return(raw)
