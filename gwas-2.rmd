...
>>>>>>> 9689d7a9a869a3762a138670afd484a8a90dbaf2
...
Below is code that imputes using the mode. I also check and convert any `snpStats` imputed values to the nearest integer values where necessary. This takes about 45 minutes (which is annoying), but may be worthwhile if:
1) your data isn't very big, or 2) if using the PLINK formats is of interest. 

Although the overall structure is similar to the mean-imputation code above, there are some key differences. Here, we will also have to re-check for monomorphic SNPs and potentially re-subset our data if we use this type of imputation. We also need to loop over all of the SNPs to check for non-integer values - I can't simply loop over the chunk with missingness as I did in the previous code.
