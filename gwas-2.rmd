...
A common method of dealing with SNPs with missing data is imputation. There are two main reasons that one would use imputation:

1. To replace missing SNP values with what these values are predicted to be, based on a person's surrounding SNP values that are not missing. 
2. To infer values for SNPs that were not measured at all. This would be the case if one was to merge data from studies that examined different loci. 
