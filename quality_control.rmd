...
PLINK is a **command line tool** (i.e., it is software that you typically run in the terminal/shell). [PLINK](https://academic.oup.com/gigascience/article/4/1/s13742-015-0047-8/2707533?login=false) was developed by researchers at Harvard, and it is widely used in genetics research. There are multiple versions of PLINK -- for our tutorial, version 1.9 is fine (this is the version cited in the methods paper linked above). You can download PLINK 1.9 for free at [this site](https://www.cog-genomics.org/plink/1.9/) -- make sure to pick the download that matches your operating system. 

Next, you need to choose a folder where you will save the PLINK software on your computer. 

  - If you are already a command-line user: the typical place to put this would be `~/.local/bin` -- but you can choose wherever you'd like. 
  - If you are new to the command line, it's ok for now to save PLINK in the same folder where you are running this tutorial. When we get to later sections of this tutorial, you will need to tell the computer 'where to look' for the *executable* file for PLINK - that is, your `R` session has to know where `plink.exe` is located. You can run `getwd()` in your `R` console right now -- by default, this is where your Rmarkdown or R script document will 'look' for PLINK. If you will use PLINK on multiple projects, pick a central location on your computer (i use `~/bin` on my Mac) and save `plink.exe` here. Then, when you need to do something with PLINK, you can supply the filepath `~/myfolder/plink`. Learning the command line is worth it's own tutorial -- check out [this one](https://missing.csail.mit.edu/2020/course-shell/) to get started learning more. 

To ensure that PLINK has installed correctly, run something like this: 

```{r eval=FALSE}
system("plink --version")
```

The `system()` command is a way for you to run commands via `R`. **Note**: you may need to tell your computer where to look for the `plink` executable file -- for example, if you downloaded PLINK and saved it to `~/Downloads/`^[Note: the `~` just stands for your home directory], you will need to run: 
...
We see that there are `r nrow(qc$genotypes)` samples and `r ncol(qc$genotypes)` variants represented in our new quality controlled data.
